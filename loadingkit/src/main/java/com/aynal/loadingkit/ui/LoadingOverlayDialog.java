...
/**
 * Overlay dialog variant (kept for backward compatibility).
 * Recommended: use OverlayHost/OverlayDimView instead of dialogs for overlays.
 */
public class LoadingOverlayDialog extends Dialog {
    ...
    public LoadingOverlayDialog(@NonNull Context context, LoadingOptions opts) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.lk_overlay);

        if (getWindow() != null) {
            WindowManager.LayoutParams lp = getWindow().getAttributes();
            lp.dimAmount = opts.dimAmount;
            getWindow().setAttributes(lp);

            if (!opts.blockTouches) {
        ...
        LinearLayout card = findViewById(R.id.lkCard);
        ...
        // ✅ TEXT loader uses lkMessage only (animated)
